...
import { 
  ...
  FiUser, FiChevronRight, FiEye, FiPhone
} from 'react-icons/fi';
...
const StaffDashboard = () => {
  ...
  const addPatientToQueue = async (patientId, appointmentId) => {
    try {
      ...
      const nextQueueNumber = queueError ? 1 : (queueData?.queue_number || 0) + 1;
                  ...
                  <tr key={appointment.id} className={appointment.is_emergency ? 'bg-red-50' : ''}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      {appointment.status === 'confirmed' && (
                        <button 
                          onClick={() => addPatientToQueue(appointment.patients.id, appointment.id)}
