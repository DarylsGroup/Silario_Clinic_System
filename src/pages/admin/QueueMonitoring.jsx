...
const QueueMonitoring = () => {
  ...
  const fetchQueueData = async () => {
    ...
    try {
      ...
      if (selectedBranch !== 'all') {
        queueQuery = queueQuery.eq('branch', selectedBranch);
      }
      ...
      // Format queue data with appointment details
      let formattedQueue = [];
      ...
      if (queueData && queueData.length > 0) {
        formattedQueue = queueData.map(item => {
          ...
          // Get branch either from queue entry or appointment
          const branch = item.branch || (matchingAppointment ? matchingAppointment.branch : 'N/A');
      ...
      if (selectedBranch !== 'all') {
        activityQuery = activityQuery.eq('branch', selectedBranch);
      }
